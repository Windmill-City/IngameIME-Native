...
class InputContext(defaultFontHeight: Int) : IInputContext {
    val nativeContext: libtf_pInputContext = memScoped {
        with(alloc<libtf_pInputContextVar>()) {
            libtf_create_ctx(ptr).succeedOrThr("libtf_create_ctx")
            value!!
        }
    }
    ...
    override var uiLess: Boolean
        get() {
            return memScoped {
                ...
                libtf_get_full_screen(nativeContext, curState.ptr).succeedOrThr("libtf_get_full_screen")
                curState.value
            }
        }
        set(value) {
            libtf_set_full_screen(nativeContext, value).succeedOrThr("libtf_set_full_screen")
        }
    ...
    override var disposed: Boolean = false
        set(value) {
            if (!value) throw IllegalStateException("Context has disposed")

            //Dispose native context
            libtf_dispose_ctx(nativeContext).succeedOrThr("libtf_dispose_ctx")
    ...
    init {
        ...
        inputProcessor = memScoped {
            ...
            libtf_get_active_input_processor(profile.ptr).succeedOrThr("libtf_get_active_input_processor")
            object : ListenableHolder<IInputProcessorProfile>(profile.toWrappedProfile()) {
                override fun setProperty(newValue: IInputProcessorProfile) {
                    libtf_set_active_input_processor((newValue as InputProcessorProfile).nativeProfile).succeedOrThr("libtf_set_active_input_processor")
        ...
        imState = memScoped {
            val curState: BooleanVar = this.alloc()
            libtf_get_im_state(nativeContext, curState.ptr).succeedOrThr("libtf_get_im_state")
        ...
        conversionMode = memScoped {
            ...
            libtf_get_conversion_mode(nativeContext, curMode.ptr).succeedOrThr("libtf_get_conversion_mode")
        ...
        sentenceMode = memScoped {
            ...
            libtf_get_sentence_mode(nativeContext, curMode.ptr).succeedOrThr("libtf_get_sentence_mode")
    ...
    fun onWindowFocusMsg(hWnd: HWND, msg: UInt) {
        libtf_on_focus_msg(nativeContext, hWnd, msg).succeedOrThr("libtf_on_focus_msg")
    }
}