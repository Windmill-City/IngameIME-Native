...
object RomanMode : IRomanMode, ITfConversionMode {
    ...
    override fun accept(state: IInputState): Boolean {
        return when (state) {
            is HiraganaMode, is KatakanaMode -> false
            else -> true
        }
    }
    ...
    override fun check(mode: libtf_ConversionMode): Boolean {
        return mode.toInt() and TF_CONVERSIONMODE_ROMAN == 1
    }
    ...
    override fun onApplyState(context: IInputContext) {
        (context as InputContext).also {
            memScoped {
                ...
                mode.value =
                    mode.value and TF_CONVERSIONMODE_KATAKANA.toUInt().inv() or TF_CONVERSIONMODE_ROMAN.toUInt()
